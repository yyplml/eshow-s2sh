	...
	*/
	...
	var Facebox = function(_options) {		
		var options = $extend({
			...
			url: false,
			ajaxErrorMessage: '<h3>404错误</h3><p>请求的文件没有被找到。</p>',
			ajaxDelay: 800,
			width: 370,
			height: 'auto',
			title: false,
		...
		//定义Body leida
		var bodyW = document.body.offsetWidth; // mbox.getSize().x
		var bodyH = document.body.offsetHeight; // mbox.getSize().y
		...
		this.show = function() {
			...
			//设置Body灰掉 leida
			var hide = new Fx.Morph(body, {duration: 400});
			hide.set({
		...
		this.close = function() {
			...
			body.setAttribute("id", "tmpBodyId");
			var overlay = $$('#tmpBodyId .bodyOverlay'); // mbox.getElements('.faceboxOverlay');
			body.setAttribute("id", "");
			if (overlay.length > 0)
				body.removeChild(overlay[0]);
			var fx = new Fx.Morph(body);
			fx.set({opacity: 0});
		}
