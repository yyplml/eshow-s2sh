package com.logo.eshow.components.compass;
...
import com.logo.eshow.components.compass.AdvancedSearchCommand.CompassSort;
...
public class CompassSearchService implements InitializingBean {
	...
	public CompassSearchResults search(final CompassSearchCommand command,
		...
		if (!StringUtils.hasText(command.getQuery())
				&& command.getCompassQuery() == null) {
			return new CompassSearchResults(new CompassHit[0], 0, 0);
		}
		return (CompassSearchResults) compassTemplate
	...
	protected CompassSearchResults performSearch(
		...
		CompassSearchResults searchResults = new CompassSearchResults(
				detachedHits.getHits(), time, hits.length());
