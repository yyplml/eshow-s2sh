...
public class GenericDaoHibernate<T, PK extends Serializable> extends
	...
	public List<T> list(final EnhancedRule enhanceRule) {
		...
		logger.debug("Get list of object whose class name is:"
				+ this.persistentClass.getName() + " by enhanced rule");
		if (enhanceRule == null) {
			return getAll();
		}
		return enhanceRule.getCriteria(this.persistentClass, getSession(false))
				.list();
	}
	...
	public T uniqueResult(final EnhancedRule enhanceRule) {
		...
		return (T) enhanceRule.getCriteria(this.persistentClass,
				getSession(false)).uniqueResult();
	}
	...
	public long count(final Class<T> clazz, final EnhancedRule enhanceRule) {
		...
		return getCount(enhanceRule.getCountCriteria(clazz, getSession(false))
				.uniqueResult());
	}
	...
	public long countAll(Class<T> clazz) {
		...
		List<T> list = list(" select count(*) from " + clazz.getName(),
				new Object[] {});
	...
	/**
	 * set the JDBC parrten parameters to Query object (from Entity entity where
	 * entity.id=?)
	 * 
	 * @param query
	 *            Query object
	...
	/**
	 * set the Hibernate parrten parameters to Query object (from Entity entity
	 * where entity.id=:id)
	 * 
	 * @param query
	 *            Query object
	 ...
	 * @return int
	 */
	public int deleteByNamedQuery(String queryName, String[] name,
			Object[] params) {

		return delete(setParameters(getQueryByName(queryName), name, params)
				.scroll());
	}
	...
	public T uniqueResultByNamedQuery(String queryName, Object[] params) {
		...
		return (T) setParameters(getQueryByName(queryName), params)
				.uniqueResult();
	}
	...
	public List<T> listByNamedQuery(String queryName, String[] name,
			Object[] params) {
	...
	public T uniqueResultByNamedQuery(String queryName, String[] name,
			Object[] params) {

		return (T) setParameters(getQueryByName(queryName), name, params)
				.uniqueResult();
	}

	public T uniqueResult(String hql, String[] name, Object[] params) {

		return (T) setParameters(getQueryByHQL(hql), name, params)
				.uniqueResult();
	}

	public int executeUpdateByNamedQuery(String updateQueryName, String[] name,
			Object[] params) {

		return setParameters(getQueryByName(updateQueryName), name, params)
				.executeUpdate();
	}
	...
	public Page<T> page(String queryName, Object[] params, int offset,
	...
	public Page<T> page(String queryName, String[] name, Object[] params,
		...
		query.setMaxResults(pageSize);
		List<T> list = query.list();
		int total = count(getQueryByName(queryName).getQueryString(), name,
				params);
		Page<T> page = new Page<T>(offset, pageSize);
		page.setTotal(total);
		page.setDataList(list);
	...
	private int count(String hql, String[] name, Object[] params) {
		...
		return getCount(setParameters(getQueryByHQL(getCountHQL(hql)), name,
				params).uniqueResult());
	}
	...
	private int count(String hql, Object[] params) {
		...
		return getCount(setParameters(getQueryByHQL(getCountHQL(hql)), params)
				.uniqueResult());
	}
	 ...
	 * 
	 */
	static final String ORDER_BY_SQL_PATTERN = "^(.+)[\\s|\\)]"
			+ ORDER_BY_PATTERN + "(.*)$";
	...
	private String getCountHQL(String query) {
		...
		String countStr = (idx1 < idx2) ? buildCountString(query.substring(
				idx1, idx2)) : buildCountString("");
	...
	private String buildCountString(String subQuery) {
		...
		while (startIdx < chars.length) {
			...
			startIdx = subQuery.substring(startIdx).toUpperCase().indexOf(
					"DISTINCT") + 8;
		}
	...
	public Page<T> pageByHQL(String hql, Object[] params, int offset,
			int pageSize) throws DAOException {
	...
	public Page<T> pageByHQL(String hql, String[] name, Object[] params,
			int offset, int pageSize) throws DAOException {
